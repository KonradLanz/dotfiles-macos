...
- **No `sort -h`** (human-readable sort) — use `sort -n` with KB values
- **No `sort -h` at all** — `-h` flag not in BusyBox v1.24 `sort`
...
- **`crontab -l`** and `cat /etc/config/crontab` produce identical output — they are the same file
...
# BAD — 'which' not always available
if which vim; then ...
```
...
```sh
#!/bin/sh        # Use for scripts that must run on QNAP/Alpine/BusyBox
#!/bin/bash      # Use only when bash features are required (arrays etc.)
#!/usr/bin/env bash  # Portable bash — finds Homebrew bash on macOS
#!/usr/bin/env zsh   # macOS-specific scripts
```
...
- `bootstrap-foundation/scripts/clone-all.sh` — NFS df workaround in production
- `qnap-storage-advisor/docs/storage-layout.md` — QNAP volume topology
- `README-shell-performance.md` — zsh startup time and shell performance
